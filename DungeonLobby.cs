...
namespace MMOG
{
    public partial class DungeonLobby
    {
        ...
        public DungeonLobby(int _lobbyID, Player _lobbyOwner, LOCATIONS _dungeonLocation, int _maxPlayersCapacity = 2)
        {
            ...
            listaGateow = new List<DungeonGate>()
            {
            new DungeonGate(LobbyID,1,DUNGEONS.DUNGEON_2,
                new Vector3(-6,8,2),
                new List<Vector3>()
                {
                    new Vector3(-8,8,0),
            ...
            new DungeonGate(LobbyID,2,DUNGEONS.DUNGEON_2,
                new Vector3(8,-2,2),
                new List<Vector3>()
                {
                    new Vector3(8,-2,0),
            ...
            (
                _lobbyID: LobbyID,
                _gateID: 3,
                _location: DUNGEONS.DUNGEON_2,
                _mainGateTilePositionsOnMap:new Vector3(-5,-12,2),
                _positionsOnMap: new List<Vector3>()
                {
                    new Vector3(-5,-12,0),
                    new Vector3(-6,-12,0),
                    new Vector3(-7,-12,0)
                ...
                _gateStatus: true,
                _triggerPosition: new Vector3(-3,-10,2)
            )
            // //---------------------------------------------------------------------------
        ...
        private void EnableClodesGates(DUNGEONS dungeonLoc)
        {
            ...
            foreach (DungeonGate gate in listaGateow.Where(g => g.Location == dungeonLoc))
            {
                ...
                gate.GroundPositionsOnMap.ForEach(p => DungeonMAPDATA_Ground.Remove(p));
            }
        }
